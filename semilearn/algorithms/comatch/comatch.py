...
class CoMatch_Net(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        logits = self.backbone(feat, only_fc=True)  # 类概率
        feat_proj = self.l2norm(self.mlp_proj(feat))    # 低维嵌入
...
def comatch_contrastive_loss(feats_x_ulb_s_0, feats_x_ulb_s_1, Q, T=0.2):
    ...
    sim = torch.exp(torch.mm(feats_x_ulb_s_0, feats_x_ulb_s_1.t())/ T)  # 强增强0和强增强1的相似性矩阵
    sim_probs = sim / sim.sum(1, keepdim=True)  # 相似性矩阵归一化
    # contrastive loss
    loss = - (torch.log(sim_probs + 1e-7) * Q).sum(1)   # 交叉熵损失
    loss = loss.mean()  
    return loss
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w, x_ulb_s_0, x_ulb_s_1):
        ...
        with self.amp_cm():
            ...
            with torch.no_grad():
                ...
                probs = self.compute_prob(logits_x_ulb_w)   # 对输出进行sotfmax
                # distribution alignment
                probs = self.call_hook("dist_align", "DistAlignHook", probs_x_ulb=probs.detach())   # 分布对齐策略 Algorithm1 Line4

                probs_orig = probs.clone()
                # memory-smoothing 
                if self.epoch > 0 and self.it > self.queue_batch:
                    A = torch.exp(torch.mm(feats_x_ulb_w, self.queue_feats.t()) / self.T)       # 衡量待计算样本和记忆库里某个样本的相对距离 Algorithm1 Line7
                    A = A / A.sum(1, keepdim=True)                    # 归一化
                    probs = self.smoothing_alpha * probs + (1 - self.smoothing_alpha) * torch.mm(A, self.queue_probs)    # 确定伪标签 Algorithm1 Line9
                
                mask = self.call_hook("masking", "MaskingHook", logits_x_ulb=probs, softmax_x_ulb=False)    
                
                feats_w = torch.cat([feats_x_ulb_w, feats_x_lb],dim=0)   # 拼接无标签弱增强和有标签的嵌入向量
                probs_w = torch.cat([probs_orig, F.one_hot(y_lb, num_classes=self.num_classes)],dim=0)  # 拼接无标签弱增强的预测和有标签的预测
            ...
            # pseudo-label graph with self-loop 伪标签矩阵
            ...
            total_loss = sup_loss + self.lambda_u * unsup_loss + self.lambda_c * contrast_loss  # + 1.0 * mcc_loss
