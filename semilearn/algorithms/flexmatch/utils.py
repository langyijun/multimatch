...
class FlexMatchThresholdingHook(MaskingHook):
    ...
    def __init__(self, ulb_dest_len, num_classes, thresh_warmup=True, *args, **kwargs):
        ...
        self.selected_label = torch.ones((self.ulb_dest_len,), dtype=torch.long, ) * -1   #初始化所有未标记数据的un为-1
        self.classwise_acc = torch.zeros((self.num_classes,))

    @torch.no_grad()
    def update(self, *args, **kwargs):
        pseudo_counter = Counter(self.selected_label.tolist())  # selected_label中从-1到num_label都有多少个样本
        if max(pseudo_counter.values()) < self.ulb_dest_len:  # not all(5w) -1
            if self.thresh_warmup:
                for i in range(self.num_classes):
                    self.classwise_acc[i] = pseudo_counter[i] / max(pseudo_counter.values())    #论文公式(11)
            else:
                wo_negative_one = deepcopy(pseudo_counter)
                if -1 in wo_negative_one.keys():
                    wo_negative_one.pop(-1)
                for i in range(self.num_classes):
                    self.classwise_acc[i] = pseudo_counter[i] / max(wo_negative_one.values())    #论文公式(11)
    ...
    def masking(self, algorithm, logits_x_ulb, idx_ulb, softmax_x_ulb=True, *args, **kwargs):
        ...
        mask = max_probs.ge(algorithm.p_cutoff * (self.classwise_acc[max_idx] / (2. - self.classwise_acc[max_idx])))  # convex 论文公式(12)
