...
class FreeMatchThresholdingHook(MaskingHook):
    ...
    def update(self, algorithm, probs_x_ulb):
        ...
        if algorithm.use_quantile:
            ...
            self.time_p = self.time_p * self.m + (1 - self.m) * max_probs.mean()    # 论文公式(5)
        
        if algorithm.clip_thresh:
            self.time_p = torch.clip(self.time_p, 0.0, 0.95)

        self.p_model = self.p_model * self.m + (1 - self.m) * probs_x_ulb.mean(dim=0)   # 论文公式(6)
        hist = torch.bincount(max_idx.reshape(-1), minlength=self.p_model.shape[0]).to(self.p_model.dtype)  # 直方图，横轴为每一类，纵轴为该类出现的次数
